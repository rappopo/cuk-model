  ...
  const action = (opt) => {
    return new Promise((resolve, reject) => {
      ...
      helper('core:configLoad')(dir, base)
        .then(result => {
          src = result
          const pkgs = _.filter(helper('core:pkgs')(), p => p.id !== opt.pkg.id)
          return Promise.map(pkgs, p => {
            const dirMerge = path.join(p.dir, 'cuks', opt.pkg.id, 'extend', 'model', 'schema')
            return helper('core:configLoad')(dirMerge, base)
          })
        })
        .then(merge => {
